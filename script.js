...
let state;
try{
  state={...defaults,...JSON.parse(localStorage.getItem("rainbowBunsikState")||"{}")};
}catch(e){state={...defaults}}
if(!Array.isArray(state.ingredients))state.ingredients=[];
if(!Array.isArray(state.upgrades))state.upgrades=[];
if(typeof state.shopName!=="string"||!state.shopName.trim())state.shopName="무지개 분식";

const $=id=>document.getElementById(id);
const won=n=>Number(n).toLocaleString("ko-KR");
function save(){try{localStorage.setItem("rainbowBunsikState",JSON.stringify(state))}catch(e){}}
...
function render(){
 ...
 if(next){
   ...
   $("next").textContent="👑 최고 레벨 달성!";
   $("progressText").textContent="최고 레벨 달성!";
   $("bar").style.width="100%";
 }
 renderCustomers();
 const text=state.ingredients.length?state.ingredients.join(" → "):"재료를 넣어주세요";
 $("potText").textContent=(state.order==="떡볶이")?text:"떡볶이를 선택하면 재료가 보여요";
 $("roseText").textContent=(state.order==="로제떡볶이")?text:"로제떡볶이를 선택하면 재료가 보여요";
...
function renderCustomers(){
 const orders=Object.keys(menu),faces=["👧🏻","👦🏻","👩🏻","🧒🏻","👨🏻","👩🏼"],names=["세아","민준","지우","하윤","도윤","서윤"];
...
document.querySelectorAll("[data-fry]").forEach(b=>b.onclick=()=>{
 ...
 setTimeout(()=>{
   if(state.fryer===item){
     state.fryer=null;state.ready=item;save();render();toast(`🎉 ${item} 완성!`);
   }
 },state.upgrades.includes("fryer")?1200:2200);
});

$("odeng").onclick=()=>{
 if(state.order!=="오뎅"){toast(`현재 손님의 주문은 ${state.order}이에요.`);return}
 state.ready="오뎅";state.ingredients=[];state.fryer=null;save();render();toast("🍢 오뎅을 담았어요!");
};

$("sell").onclick=()=>{
 if(!state.ready){toast("먼저 음식을 완성해주세요!");return}
 if(state.ready!==state.order){toast("주문과 음식이 달라요!");return}
 const p=menu[state.ready].price;
 state.money+=p;state.ready=null;state.ingredients=[];state.fryer=null;state.currentCustomer++;
...
$("shop").onclick=()=>{
 ...
 <div class="shopGrid">${upgrades.map((u,i)=>{
   ...
   <button class="buy" ${owned||!unlocked||state.money<u.price?"disabled":""} onclick="buy('${u.id}')">${owned?"완료":!unlocked?"상위 레벨 필요":"구매하기"}</button></div>`;
 }).join("")}</div>`);
};
...
window.saveName=()=>{
 const n=$("newName").value.trim()||"무지개 분식";
 state.shopName=n;save();closeModal();render();toast(`가게 이름: ${n}`);
};
